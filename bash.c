...
**No ONENOTES tenho alguns prints importantes**
...
->FILTROS
	...
	-» users que estão no sistema: cat /etc/passwd

	-» ver conteudo de um ficheiro: cat ficheiro.txt
...
-» cat /etc/passwd mostra todos os users registados
...
-»Para contar
	-> wc 
		-> conta deste modo: -l(lines)  -w(words)  -c(bytes) -m (caracteres)
...
-»Para saber o tamanho de o ficheiro
...
-» who
	-> mostra quem está ligado na máquina

-» finger
	-> informação parecida ao who mas com mais pormenores

-» whoami
	-> diz o nome do user que estou a usar
...
-» chmod *g* ou *u* ou *o* + ou - ou = *r* ou *w* ou *x*
	-> estamos a dar(+) ou tirar(-) permissões ao grupo (g), user (u) ou outro (o) de ler (r) ou escrever (w) ou executar (x)
...
-» head -n x - mostra as x linhas desde o inicio, as primeiras x linhas == head -n +x (FAZ O MESMO)   

-» tail -n x - mostra as x linhas do fim do ficheiro, as ultimas x linhas
-» tail -n +x - mostra a partir da linha x até ao fim
	...
	-> cat texto.txt | tail -n 1 | cut -c15-20 -- na ultima linha, vai buscar apenas do caracter 15 ao 20
		...
		-> cat texto.txt | tail -n 1 | cut -d"," -f3 -- corta a partir de uma virgula, no 3º campo ou -f1,3 seria o 1º e 3º campo
			...
			-> tr -s " " tiramos espaços a mais


	-> cat texto.txt | tail -n +3 : da limha 3 até ao fim

-» sort: para ordernar uma pesquisa -> só sort apenas ordena por ordem alfabetica
	-> sort -n(numeros - tamanhos) ou -r(ordernar revertido) ou -t(delimitador) ou -k(campo) ou -u(unique)
...
-» aspas protegem caracteres
 ...
 -» diretoria pessoal : ls -l /home/rafael 

 -» OR -- \|
 -» AND -- .*

-» lspci - ver os dispositivos que estão na máquina

-» \n para dar enter num ficheiro de texto, por exemplo "rafael" e "ribeiro" em duas linhas diferentes -> echo -e "rafael\nribeiro" > ficheiro.txt

-» ps al para um listagem longa de todos os processos

-» uname - nome do sistema